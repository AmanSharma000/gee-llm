...
\begin{tabular*}{\tblwidth}{@{}P{1.8cm}@{\extracolsep{\fill}}P{1.5cm}@{\extracolsep{\fill}}P{1.8cm}@{\extracolsep{\fill}}P{1.1cm}@{\extracolsep{\fill}}P{1.8cm}@{\extracolsep{\fill}}P{1.1cm}@{\extracolsep{\fill}}P{1.5cm}@{\extracolsep{\fill}}P{1.4cm}@{}}
\toprule
Framework & Domain & Retrieval / Augment. & Live Exec. & Feedback Source & Sci. Check & Grounded Interp. & Compute Footprint \\
\midrule
Zero-Shot LLMs & General Code & None & $\times$ & None & $\times$ & $\times$ & Low (Inference) \\
\addlinespace
GeoGPT & General GIS & None (Tool Routing) & $\checkmark$ (Tools) & Tool exec. logs & $\times$ & $\times$ & Medium (Cloud API) \\
\addlinespace
GeoCode-GPT & Geospatial Code & None (Fine-tune) & $\times$ & None & $\times$ & $\times$ & High (GPU) \\
\addlinespace
AutoGEEval & GEE Scripts & None & $\checkmark$ (Eval) & None (Scoring) & $\times$ & $\times$ & Low (API-based) \\
\addlinespace
\textbf{GEE-LLM} & GEE Scripts & Sparse Lexical (CPU) & $\checkmark$ (Live) & Regex GEE except. & $\checkmark$ (Physical) & $\checkmark$ (Numeric) & Low (CPU edge) \\
\bottomrule
\end{tabular*}
\end{table}
