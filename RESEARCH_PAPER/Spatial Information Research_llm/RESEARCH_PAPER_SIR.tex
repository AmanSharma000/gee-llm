...
\toprule
Framework & Target Domain & Retrieval RAG & Live Server Execution & Feedback Source & Scientific Check & Grounded Interpretation & Compute Footprint \\
\midrule
Zero-Shot LLMs & General Code & None & $\times$ & None & $\times$ & $\times$ & Low (Inference) \\
\addlinespace
GeoGPT & General GIS & Dense Vector (GPU) & $\checkmark$ (Tools) & Tool execution logs & $\times$ & $\times$ & High (GPU) \\
\addlinespace
GeoCode-GPT & General GIS & Dense Vector (GPU) & $\times$ & Interpreter stack trace & $\times$ & $\times$ & High (GPU) \\
\addlinespace
...
The fine-grained active bin pixel count distributions passed from the GEE execution database to the secondary LLM are presented in Table~\ref{tbl:delhi_mumbai_histogram}.
...
From these inputs, the interpretation engine generated the following scientific narrative:
\begin{quote}
\itshape
